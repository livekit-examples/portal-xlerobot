...
class XLerobot(Robot):
    ...
    def calibrate(self) -> None:
        if self.calibration:
            # Calibration file already loaded by base class: offer to reuse it.
            user_input = input(
        ...
        # See the right-side block below for the rationale; mirror here
        # so ``left_arm_wrist_roll`` doesn't get swept into an encoder-wrap
        # state either.
        left_full_turn_motor = [
            motor for motor in left_motors if any(keyword in motor for keyword in ["wrist_roll"])
        ]
        left_sweep_motors = [motor for motor in left_motors if motor not in left_full_turn_motor]

        print(
            f"Move all left arm and head joints except '{left_full_turn_motor}' sequentially through their "
            "entire ranges of motion.\nRecording positions. Press ENTER to stop..."
        )
        range_mins, range_maxes = self.bus1.record_ranges_of_motion(left_sweep_motors)
        ...
        # "full_turn_motor" here is a calibration-flow label, not a claim
        # about the joint's physical range: it means "skip the sweep step
        # and set this motor's range to [0, 4095]". The alternative — a
        # proper sweep — lands in a broken state whenever the sweep path
        # crosses the encoder's 0/4095 wrap point, because
        # ``record_ranges_of_motion`` just tracks raw min/max and has no
        # wrap-aware logic. Matches so_follower.py:131 exactly.
        full_turn_motor = [
            motor
    ...
    def configure(self):
        # Arm / head / gripper motor config: mirror so_follower.py:155 exactly.
        # P=16, I=0, D=32. Grippers additionally get Max_Torque_Limit=500,
        # Protection_Current=250, Overload_Torque=25 as a burnout safeguard
        # (an active gripper can stall indefinitely on a gripped object).
        # Base wheels stay in velocity mode as before.
        self.bus1.disable_torque()
        self.bus1.configure_motors()
        self.bus2.disable_torque()
        self.bus2.configure_motors()

        def _configure_position_motor(bus, name: str) -> None:
        ...
        for name in self.left_arm_motors + self.head_motors:
            _configure_position_motor(self.bus1, name)
        for name in self.right_arm_motors:
            _configure_position_motor(self.bus2, name)
    ...
    def get_camera_observation(self):
        ...
        for cam_key, cam in self.cameras.items():
            start = time.perf_counter()
            try:
                # ``read_latest`` returns the most recent frame from the
                # background capture thread without blocking. ``async_read``
                # (the previous default) *waits* for a fresh frame to
                # arrive, which at 30 fps meant ~33 ms per tick of pure
                # wait time on the first camera read. For teleop /
                # publishing we're fine with "whatever the camera thread
                # has right now"; if the buffer is empty or the frame is
                # older than the default max_age_ms (500), the camera
                # raises and we drop that key (downstream treats missing
                # keys as "no new frame for this track" and reuses last
                # published).
                obs_dict[cam_key] = cam.read_latest()
            except Exception as exc:
                # Dead / stalled camera, or no fresh frame yet. Neither
                # should take down the teleop loop. Drop the key; the
                # LiveKit publisher skips missing entries.
                logger.warning("camera %s read failed: %s — dropping this tick", cam_key, exc)
            dt_ms = (time.perf_counter() - start) * 1e3
    ...
    def send_action(self, action: RobotAction) -> RobotAction:
        ...
        if self.config.max_relative_target is not None:
            ...
            # Goal dicts use ".pos"-suffixed keys (e.g. "left_arm_shoulder_pan.pos")
            # but ``bus.sync_read`` returns plain motor-name keys
            # (e.g. "left_arm_shoulder_pan"), so strip the suffix when
            # looking up present positions.
            goal_present_pos = {
